...
API Endpoints
POST /api/register: Register a new user by providing username, email, and password.
POST /api/login: Authenticate a user by providing username/email and password. Returns an access token upon successful login.
POST /api/refresh-token: Refresh an expired access token using a valid refresh token.
POST /api/reset-password: Initiate the password reset process for a user by sending a reset link to their email.
POST /api/change-password: Change the user's password after providing the old password and the new password.
...
License
The Authentication API is open-source software licensed under the MIT License.

Contact
For any questions or inquiries, please contact the project maintainer at augustine.onaodowan@gmail.com.
